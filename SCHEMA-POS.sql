...
GRANT CREATE ANY DIRECTORY TO ROOT;
GRANT EXECUTE ON UTL_FILE TO ROOT;
...
GRANT EXECUTE ON ROOT.SP_EXPORTAR_VENTAS TO POS_EMPLEADO;
...
CREATE OR REPLACE PROCEDURE SP_EXPORTAR_VENTAS
(
P_FECHA_INICIO ROOT.VENTA.FECHA_REGISTRO%TYPE,
P_FECHA_FIN ROOT.VENTA.FECHA_REGISTRO%TYPE,
P_DIR IN VARCHAR2,
...
IS
 ...
 SELECT * FROM ROOT.VW_VENTAS_PARA_REPORTE;
 CUR_VENTAS_RECORD CUR_VENTAS%ROWTYPE;
 V_OUTPUT UTL_FILE.FILE_TYPE;
BEGIN
  ROOT.ROOT_REPORTE.V_REPORTE_FECHA_INICIO:=P_FECHA_INICIO;
  ROOT.ROOT_REPORTE.V_REPORTE_FECHA_FIN:=P_FECHA_FIN;
  --MODIFICANDO DIRECTORIO DONDE SE GUARDARA EL CSV
  EXECUTE IMMEDIATE 'CREATE OR REPLACE DIRECTORY CSV_DIRECTORY AS '''||P_DIR||'''';
  V_OUTPUT:=UTL_FILE.FOPEN('CSV_DIRECTORY',P_NOMBRE_ARCHIVO,'w');
  OPEN CUR_VENTAS;
  LOOP
    FETCH CUR_VENTAS INTO CUR_VENTAS_RECORD;
    EXIT WHEN CUR_VENTAS%NOTFOUND;
    UTL_FILE.PUT_LINE(V_OUTPUT,
    CUR_VENTAS_RECORD.VENTA_ID||','||
    CUR_VENTAS_RECORD.FECHA_REGISTRO||','||
    CUR_VENTAS_RECORD.VENTA_BRUTA||','||
    CUR_VENTAS_RECORD.TOTAL_IMPUESTO||','||
    CUR_VENTAS_RECORD.TOTAL_DESCUENTO||','||
    CUR_VENTAS_RECORD.TOTAL_COSTO||','||
    CUR_VENTAS_RECORD.PAGO_CLIENTE||','||
    CUR_VENTAS_RECORD.CAMBIO||','||
    CUR_VENTAS_RECORD.CLIENTE_ID||','||
    CUR_VENTAS_RECORD.USUARIO_ID
    );
  END LOOP;
